...
0.- La presencia de tick está relacionada positivamente con la presencia de animales pero también con la presencia de maleza (weeds) y de peromyscus.

1.- La presencia de maleza (weeds) es mayor cuanto más hierba haya (grass) y menos cuanto más highcan haya. 

2.- A más proporción de lowcan menos plantas herbáceas hay.

3.- Cuanta menos hierba (grass), mayor cantidad de plantas ornamentales hay.

4.- La presencia de cats, raccoons y woodchucks es menor si hay hierba (weeds).

5.- A mayor proporción de suelo limpio (imperv), mayor cantidad de raccoons, woodchuchs y squirrels.

6.- Hay más woodchucks cuanta mayor cantidad de evergreen y suelo limpio haya.

7.- Cuanta más proporción de low can haya menos ardillas hay pero si el suelo está limpio hay más.

```{R mathematical_model}

fitglm.null <- glm(Ticks_label ~ 1, family = binomial(logit), data = datos)
summary(fitglm.null)
fitglm <- glm(Ticks_label ~ Raccon_VisR + Deer_VisR + Woodchuck_VisR + 
         Cat_VisR + Possum_VisR + Skunk_VisR + Squirrel_VisR + Number.tubes.peromyscus + ApproxMweeds + Number.tubes.raccoon + prop.imperv, family = binomial(logit), data = datos)
...
fitglm.null <- glm(Cat_VisR ~ 1, family = gaussian, data = datos)
summary(fitglm.null)
fitglm <- glm(Cat_VisR ~ Log.brush + Vegetable + ApproxMweeds, 
...
##Raccoon
fitglm.null <- glm(Raccon_VisR ~ 1, family = gaussian, data = datos)
summary(fitglm.null)
fitglm <- glm(Raccon_VisR ~ prop.imperv + Lawn + ApproxMweeds + Log.brush, 
         family = binomial(logit), data = datos)
...
fitglm.null <- glm(Deer_VisR ~ 1, family = gaussian, data = datos)
summary(fitglm.null)
fitglm <- glm(Deer_VisR ~ Log.brush + Lawn, 
         family = binomial(logit), data = datos)
...
##Woodchuck
fitglm.null <- glm(Woodchuck_VisR ~ 1, family = gaussian, data = datos)
summary(fitglm.null)
fitglm <- glm(Woodchuck_VisR ~ ApproxMevergreen + prop.imperv + ApproxMweeds, 
         family = binomial(logit), data = datos)
...
##Squirrel
fitglm.null <- glm(Squirrel_VisR ~ 1, family = gaussian, data = datos)
summary(fitglm.null)
fitglm <- glm(Squirrel_VisR ~ Log.brush + Lawn + prop.imperv + prop.lowcan, 
         family = binomial(logit), data = datos)
